...
import { fetchProductManagerAsync, fetchUpdateProductAsync, setCategoryId } from '../productsManagerSlice'
import { productManagerSelector } from '../selector'
...
const Update = () => {
  ...
  const product = useSelector(productManagerSelector)

  useEffect(() => {
    dispatch(fetchProductManagerAsync(id))
  ...
  const onSubmit = (data) => {
    if(imgSrc){
      dispatch(fetchUpdateProductAsync({...data,image:imgSrc}))
      notify('cập nhật sản phẩm thành công')
      navigate('/admin/danh-sach')
      dispatch(setCategoryId(product.categoryId))
    }else{
